...
// (whose `result.success` is deprecated / empty by default) �?// `compileToProgram` is the documented Phase 3.6 entry point and
...
// shaderc availability (Commit 5: pre-shaderc populate point) �?// so substring assertions on `prog.sources[k]` succeed even when
...
TEST_CASE(compile_with_if_else) {
    ...
    // (the legacy `.output != empty` check was overly permissive �?    // we don't ship empty garbage to the frontend in 3.6).
    if (prog.success) {
        CHECK(!prog.sources.empty());
    }
}
