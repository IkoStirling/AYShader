...
TEST_CASE(ir_property_uniform_type_resolved_from_initializer) {
    ...
    CHECK(bgfx.success);
    CHECK_FALSE(bgfx.materialStages.empty());
    CHECK(bgfx.materialStages.front().vertex.find("uniform vec3 col") != std::string::npos);
}
...
TEST_CASE(ir_uniformblock_lowers_with_field_types) {
    ...
    // lexemeToType table �?vec3 �?VectorType(Float,3), float �?    // Float, uint �?Uint (Phase 3.3 Block 1).
    auto ir = generateIR(R"(
        uniformblock Camera {
            vec3 position
            float fov
...
TEST_CASE(ir_uniformblock_array_field_preserves_arraylength) {
    ...
    CHECK(arr->size() == 128u);
    // Element type is mat4 (MatrixType Float×4).
