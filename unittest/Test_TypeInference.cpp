...
// type directly (uvec3) when the env doesn't have the name ‚Ä?exactly
...
// Convenience: cast helper ‚Ä?extract the concrete type a TypeVar points to.
...
// ===== Scalar√óvector broadcasting (commutative) =====
//
...
// this: the result of a scalar√óvector op is the vector side, no
...
// Chained scalar√óvector (e.g. PBR `D * G * F / max(4*NdotV, 0.001)`):
...
TEST_CASE(chained_scalar_vector_vector_returns_vector) {
    InferenceEnv e;
    // ((float * vec3) * vec3) ‚Ä?left-assoc parse.
...
TEST_CASE(swizzle_x_of_ivec3_returns_int) {
    // Pin the ivec branch too ‚Ä?same code path, different element
